        ...
        <div class="overflow-x-auto overscroll-x-auto flex flex-col gap-0 sm:gap-4">
            <div class="grid gap-3 sm:flex md:justify-between md:items-center">
                <div class="flex flex-col sm:flex-row gap-1 sm:gap-2 items-start sm:items-center">
                    ...
                    <p> <?php breadcrumb(['Dashboard' => '/admin/dashboard', 'Product Categories' => null]); ?></p>
                </div>
                    ...
                    <tbody>
                        <?php foreach ($categories as $category): ?>
                            <tr class="bg-neutral-primary-soft text-neutral-600 border-b last:border-b-0">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <?= htmlspecialchars(
                                        mb_strlen($category['description'], 'UTF-8') > 50
                                        ? mb_substr($category['description'], 0, 80, 'UTF-8') . '...'
                                        : $category['description']
                                    ) ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <?php if (!empty($category['available'])): ?>
                                        <span
                                            class="inline-flex items-center rounded-md bg-indigo-100 border border-indigo-300 px-3 py-1 text-xs font-semibold text-indigo-700">
                                            Available
                                        </span>
                                    <?php else: ?>
                                        <span
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <?= date('D, d M Y', strtotime($category['created_at'])) ?>
                                </td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end items-center gap-1">
                                        <a class="py-1.5 px-2.5 inline-flex items-center gap-x-2 text-xs font-medium rounded-lg border border-gray-300 bg-gray-100 text-gray-800 hover:bg-gray-200 focus:outline-hidden focus:bg-gray-200 disabled:opacity-50 disabled:pointer-events-none"
                                            href="/admin/product-categories/<?= $category['slug_uuid'] ?>/edit">
                                            Edit
                                        </a>
                                        <form method="POST"
                                            action="/admin/product-categories/<?= $category['slug_uuid'] ?>/delete"
                                            class="inline">
                                            <input type="hidden" name="id" value="<?= $category['slug_uuid'] ?>">
                                            <button
