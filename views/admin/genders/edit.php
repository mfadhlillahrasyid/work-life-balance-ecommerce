...
<div class="max-w-full mx-auto">
    <div class="overflow-hidden">
        <div class="overflow-x-auto overscroll-x-auto flex flex-col gap-4">
            ...
            <form method="POST" action="/admin/genders/<?= $gender['slug_uuid'] ?>/update"
                class="bg-white p-5 rounded-2xl border border-neutral-200">

                <input type="hidden" name="remove_banner" value="0" id="removeBannerInput">

                <div class="flex flex-col gap-4 sm:gap-6">
                    <div class="flex flex-col gap-2">
                        <label class="block text-sm font-semibold">Title</label>
                        <input type="text" name="title" value="<?= htmlspecialchars($gender['title']) ?>" required
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                        <div class="flex flex-col gap-2">
                            ...
                            <div id="banner-dropzone"
                                data-existing-banner="<?= htmlspecialchars($gender['banner'] ?? '') ?>"
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <textarea name="description" rows="7" placeholder="Write Post Description..."
                                class="w-full border text-sm tracking-tight rounded-lg px-3 py-2 bg-neutral-50 text-neutral-700 focus:ring-2 focus:ring-indigo-700 focus:outline-none"><?= htmlspecialchars($gender['description']) ?></textarea>
                        </div>
                    </div>
